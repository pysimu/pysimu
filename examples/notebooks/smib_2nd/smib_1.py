...
class smib_1_class: 
    ...
    def update(self): 
        ...
        values += [np.zeros((12,1))]
        dt +=     [('Fy', np.float64, (9,12))]
        values += [np.zeros((9,12))]
        dt +=     [('Gx', np.float64, (12,9))]
        values += [np.zeros((12,9))]
...
def run(t,struct, mode):
    ...
    if mode==3: # algebraic equations 
        ...
        struct[it].g[7,0] = -P_t - V_inf*V_t*(-B_t_inf*sin(theta_inf - theta_t) + G_t_inf*cos(phi_inf - theta_t)) + V_t**2*(G_t0 + G_t_inf)  
        struct[it].g[8,0] = -Q_t + V_inf*V_t*(B_t_inf*cos(theta_inf - theta_t) + G_t_inf*sin(theta_inf - theta_t)) + V_t**2*(-B_t0 - B_t_inf)  
        struct[it].g[9,0] = K_a*(V_ref - v_c + v_pss) - v_f  
    ...
    if mode==11: # Fy,Gx,Gy 
        ...
        struct[it].Fy[2,0] = (X1d - X_d)/T1d0 
        struct[it].Fy[2,9] = 1/T1d0 
        struct[it].Fy[3,1] = (-X1q + X_q)/T1q0 
        struct[it].Fy[4,8] = 1/T_r 
        ...
        struct[it].Gx[7,7] = G_t_inf*V_inf*V_t*sin(phi_inf - theta_t) 
        ...
        struct[it].Gy[6,6] = -1 
        struct[it].Gy[7,5] = -1 
        struct[it].Gy[7,7] = -V_inf*V_t*(B_t_inf*cos(theta_inf - theta_t) + G_t_inf*sin(phi_inf - theta_t)) 
        struct[it].Gy[7,8] = -V_inf*(-B_t_inf*sin(theta_inf - theta_t) + G_t_inf*cos(phi_inf - theta_t)) + 2*V_t*(G_t0 + G_t_inf) 
        struct[it].Gy[8,6] = -1 
...
def initialization(struct):
    ...
    struct[0].f_ini[7,0] = Omega_b*(omega_inf - 1) 
    struct[0].f_ini[8,0] = RoCoFpu 
    ...
    struct[0].g_ini[7,0]  = -P_t - V_inf*V_t*(-B_t_inf*sin(theta_inf - theta_t) + G_t_inf*cos(phi_inf - theta_t)) + V_t**2*(G_t0 + G_t_inf)  
    struct[0].g_ini[8,0]  = -Q_t + V_inf*V_t*(B_t_inf*cos(theta_inf - theta_t) + G_t_inf*sin(theta_inf - theta_t)) + V_t**2*(-B_t0 - B_t_inf)  
    struct[0].g_ini[9,0]  = K_a*(V_ref - v_c + v_pss) - v_f  
    ...
    struct[0].Gx_ini[7,7] = G_t_inf*V_inf*V_t*sin(phi_inf - theta_t) 
    ...
    struct[0].Gy_ini[7,7] = -V_inf*V_t*(B_t_inf*cos(theta_inf - theta_t) + G_t_inf*sin(phi_inf - theta_t)) 
    struct[0].Gy_ini[7,8] = -V_inf*(-B_t_inf*sin(theta_inf - theta_t) + G_t_inf*cos(phi_inf - theta_t)) + 2*V_t*(G_t0 + G_t_inf) 
...
def ini_struct(dt,values):
    ...
    values += [np.zeros((9,9))]
    dt +=     [('Fy_ini', np.float64, (9,12))]
    values += [np.zeros((9,12))]
    dt +=     [('Gx_ini', np.float64, (12,9))]
    values += [np.zeros((12,9))]
...
if __name__ == "__main__":
    sys = {'t_end': 20.0, 'Dt': 0.01, 'solver': 'forward-euler', 'decimation': 10, 'name': 'smib_1', 'models': [{'params': {'X_d': 1.81, 'X1d': 0.3, 'T1d0': 8.0, 'X_q': 1.76, 'X1q': 0.65, 'T1q0': 1.0, 'R_a': 0.003, 'X_l': 0.15, 'H': 3.5, 'Omega_b': 376.99111843077515, 'B_t0': 0.0, 'G_t_inf': 0.0, 'T_r': 0.05, 'theta_inf': 0.0, 'T_pss_1': 1.281, 'T_pss_2': 0.013, 'T_w': 5.0, 'D': 0.0, 'K_a': 200.0, 'K_stab': 10, 'B_t_inf': -2.10448859455482, 'G_t0': 0.01, 'V_inf': 0.9008}, 'f': ['ddelta=Omega_b*(omega - 1)', 'domega = -(p_e - p_m + D*(omega - 1))/(2*H)', 'de1q =(v_f - e1q + i_d*(X1d - X_d))/T1d0', 'de1d = -(e1d + i_q*(X1q - X_q))/T1q0', 'dv_c =   (V_t - v_c)/T_r', 'dx_pss = (omega_w - x_pss)/T_pss_2', 'dx_w =  (omega - x_w)/T_w', 'dphi_inf = Omega_b*(omega_inf - 1)', 'domega_inf = RoCoFpu'], 'g': ['i_d@ v_q - e1q + R_a*i_q + i_d*(X1d - X_l)', 'i_q@ v_d - e1d + R_a*i_d - i_q*(X1q - X_l)', 'p_e@ p_e - i_d*(v_d + R_a*i_d) - i_q*(v_q + R_a*i_q) ', 'v_d@ v_d - V_t*sin(delta - theta_t)', 'v_q@ v_q - V_t*cos(delta - theta_t)', 'P_t@ i_d*v_d - P_t + i_q*v_q', 'Q_t@ i_d*v_q - Q_t - i_q*v_d', 'theta_t @(G_t0 + G_t_inf)*V_t**2 - V_inf*(G_t_inf*cos(theta_t - phi_inf) + B_t_inf*sin(theta_t - theta_inf))*V_t - P_t', 'V_t@ (- B_t0 - B_t_inf)*V_t**2 + V_inf*(B_t_inf*cos(theta_t - theta_inf) - G_t_inf*sin(theta_t - theta_inf))*V_t - Q_t', 'v_f@K_a*(V_ref - v_c + v_pss) - v_f', 'v_pss@v_pss + K_stab*(x_pss*(1/T_pss_2 - 1) - (T_pss_1*omega_w)/T_pss_2)', 'omega_w@ omega_w - omega + x_w'], 'u': {'p_m': 0.9, 'V_ref': 1.0, 'RoCoFpu': 0.0}, 'u_ini': {}, 'y_ini': ['i_d', 'i_q', 'p_e', 'v_d', 'v_q', 'P_t', 'Q_t', 'theta_t', 'V_t', 'v_f', 'v_pss', 'omega_w'], 'h': ['omega']}], 'perturbations': [{'type': 'step', 'time': 1.0, 'var': 'V_ref', 'final': 1.01}], 'itol': 1e-08, 'imax': 100, 'solvern': 1}
    syst =  smib_1_class()
