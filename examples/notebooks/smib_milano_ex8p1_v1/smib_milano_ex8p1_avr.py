...
class smib_milano_ex8p1_avr_class: 
    def __init__(self): 
...
def run(t,struct, mode):
    ...
    if mode==3: # algebraic equations 
        ...
        struct[it].g[9,0] = K_a*(-v_c + v_pss + v_ref) - v_f  
...
def initialization(struct):
    ...
    struct[0].g_ini[9,0]  = K_a*(-v_c + v_pss + v_ref) - v_f  
...
if __name__ == "__main__":
    sys = {'t_end': 20.0, 'Dt': 0.001, 'solver': 'forward-euler', 'decimation': 10, 'name': 'smib_milano_ex8p1_avr', 'models': [{'params': {'X_d': 1.81, 'X1d': 0.3, 'T1d0': 8.0, 'X_q': 1.76, 'X1q': 0.65, 'T1q0': 1.0, 'R_a': 0.003, 'X_l': 0.05, 'H': 3.5, 'D': 1.0, 'Omega_b': 314.1592653589793, 'v_0': 0.9008, 'theta_0': 0.0, 'T_r': 0.05, 'K_a': 200.0, 'T_pss_1': 1.281, 'T_pss_2': 0.013, 'T_w': 5.0, 'K_stab': 10}, 'f': ['ddelta = Omega_b*(omega - 1)', 'domega = 1/(2*H)*(p_m - p_e - D*(omega - 1))', 'de1q = 1/T1d0*(-e1q - (X_d - X1d)*i_d + v_f)', 'de1d = 1/T1q0*(-e1d + (X_q - X1q)*i_q)', 'dv_c =   (v_1 - v_c)/T_r', 'dx_pss = (omega_w - x_pss)/T_pss_2', 'dx_w =  (omega - x_w)/T_w'], 'g': ['v_1@ P_t - (v_1*v_0*sin(theta_1 - theta_0))/X_l ', 'theta_1@ Q_t + (v_1*v_0*cos(theta_1 - theta_0))/X_l - v_1**2/X_l', 'v_d@ v_1*sin(delta - theta_1) - v_d', 'v_q@ v_1*cos(delta - theta_1) - v_q', 'i_d@ v_q + R_a*i_q + X1d*i_d - e1q', 'i_q@ v_d + R_a*i_d - X1q*i_q - e1d', 'p_e@ i_d*(v_d + R_a*i_d) + i_q*(v_q + R_a*i_q) - p_e', 'P_t@ i_d*v_d + i_q*v_q - P_t', 'Q_t@ i_d*v_q - i_q*v_d - Q_t', 'v_f@K_a*(v_ref - v_c + v_pss) - v_f', 'v_pss@v_pss + K_stab*(x_pss*(1/T_pss_2 - 1) - (T_pss_1*omega_w)/T_pss_2)', 'omega_w@ omega_w - omega + x_w'], 'u': {'p_m': 0.2, 'v_ref': 1.0}, 'u_ini': {}, 'y_ini': ['theta_1', 'v_1', 'v_d', 'v_q', 'i_d', 'i_q', 'p_e', 'P_t', 'Q_t', 'v_f', 'v_pss', 'omega_w'], 'h': ['omega']}], 'perturbations': [{'type': 'step', 'time': 1.0, 'var': 'p_m', 'final': 1.01}], 'itol': 1e-08, 'imax': 100, 'solvern': 1}
    syst =  smib_milano_ex8p1_avr_class()
