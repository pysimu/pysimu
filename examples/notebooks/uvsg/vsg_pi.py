...
class vsg_pi_class: 
    ...
    def update(self): 
        ...
        dt +=     [('g', np.float64, (20,1))]
        values += [np.zeros((20,1))]
        dt +=     [('y', np.float64, (20,1))]
        values += [np.zeros((20,1))]
        dt +=     [('Fy', np.float64, (5,20))]
        values += [np.zeros((5,20))]
        dt +=     [('Gx', np.float64, (20,5))]
        values += [np.zeros((20,5))]
        dt +=     [('Gy', np.float64, (20,20))]
...
def run(t,struct, mode):
    ...
    p_s = struct[it].y[18,0] 
    q_s = struct[it].y[19,0] 


    if mode==2: # derivatives 

        dphi_s = K_p*epsilon_p + K_i*xi_p 
        dxi_p  = epsilon_p 
        dxi_q  = epsilon_q 
    ...
    if mode==3: # algebraic equations 
        ...
        struct[it].g[8,0] = -i_sd + i_sd_s*cos(phi_s) + i_sq_s*sin(phi_s)  
        struct[it].g[9,0] = -i_sd_s*sin(phi_s) - i_sq + i_sq_s*cos(phi_s)  
        struct[it].g[10,0] = -v_sd  
        struct[it].g[11,0] = K_q*(epsilon_q + xi_q/T_q) - v_sq - 1  
        struct[it].g[12,0] = v_sd*cos(phi_s) - v_sd_s - v_sq*sin(phi_s)  
        struct[it].g[13,0] = v_sd*sin(phi_s) + v_sq*cos(phi_s) - v_sq_s  
        struct[it].g[14,0] = -v_sd_s + v_si  
        struct[it].g[15,0] = -v_sq_s - v_sr  
        struct[it].g[16,0] = -epsilon_p + p_m - p_s  
        struct[it].g[17,0] = -epsilon_q - q_s + q_s_ref  
        struct[it].g[18,0] = i_sd*v_sd + i_sq*v_sq - p_s  
        struct[it].g[19,0] = i_sd*v_sq - i_sq*v_sd - q_s  
    ...
    if mode==11: # Fy,Gx,Gy 
        ...
        struct[it].Fy[0,16] = K_p 
        struct[it].Fy[1,16] = 1 
        struct[it].Fy[2,17] = 1 
    

        struct[it].Gx[2,3] = -V_g*sin(phi_g) 
        struct[it].Gx[3,3] = V_g*cos(phi_g) 
        struct[it].Gx[8,0] = -i_sd_s*sin(phi_s) + i_sq_s*cos(phi_s) 
        struct[it].Gx[9,0] = -i_sd_s*cos(phi_s) - i_sq_s*sin(phi_s) 
        struct[it].Gx[11,2] = K_q/T_q 
        struct[it].Gx[12,0] = -v_sd*sin(phi_s) - v_sq*cos(phi_s) 
        struct[it].Gx[13,0] = v_sd*cos(phi_s) - v_sq*sin(phi_s) 
        ...
        struct[it].Gy[7,4] = -1 
        struct[it].Gy[7,7] = -1 
        struct[it].Gy[8,6] = cos(phi_s) 
        struct[it].Gy[8,7] = sin(phi_s) 
        struct[it].Gy[8,8] = -1 
        struct[it].Gy[9,6] = -sin(phi_s) 
        struct[it].Gy[9,7] = cos(phi_s) 
        struct[it].Gy[9,9] = -1 
        struct[it].Gy[10,10] = -1 
        struct[it].Gy[11,11] = -1 
        struct[it].Gy[11,17] = K_q 
        struct[it].Gy[12,10] = cos(phi_s) 
        struct[it].Gy[12,11] = -sin(phi_s) 
        struct[it].Gy[12,12] = -1 
        struct[it].Gy[13,10] = sin(phi_s) 
        struct[it].Gy[13,11] = cos(phi_s) 
        ...
        struct[it].Gy[16,18] = -1 
        struct[it].Gy[17,17] = -1 
        struct[it].Gy[17,19] = -1 
        struct[it].Gy[18,8] = v_sd 
        struct[it].Gy[18,9] = v_sq 
        struct[it].Gy[18,10] = i_sd 
        struct[it].Gy[18,11] = i_sq 
        struct[it].Gy[18,18] = -1 
        struct[it].Gy[19,8] = v_sq 
        struct[it].Gy[19,9] = -v_sd 
        struct[it].Gy[19,10] = -i_sq 
        struct[it].Gy[19,11] = i_sd 
...
def initialization(struct):
    ...
    p_s = struct[0].y_ini[18,0] 
    q_s = struct[0].y_ini[19,0] 
    struct[0].f_ini[0,0] = K_i*xi_p + K_p*epsilon_p 
    ...
    struct[0].g_ini[8,0]  = -i_sd + i_sd_s*cos(phi_s) + i_sq_s*sin(phi_s)  
    struct[0].g_ini[9,0]  = -i_sd_s*sin(phi_s) - i_sq + i_sq_s*cos(phi_s)  
    struct[0].g_ini[10,0]  = -v_sd  
    struct[0].g_ini[11,0]  = K_q*(epsilon_q + xi_q/T_q) - v_sq - 1  
    struct[0].g_ini[12,0]  = v_sd*cos(phi_s) - v_sd_s - v_sq*sin(phi_s)  
    struct[0].g_ini[13,0]  = v_sd*sin(phi_s) + v_sq*cos(phi_s) - v_sq_s  
    struct[0].g_ini[14,0]  = -v_sd_s + v_si  
    struct[0].g_ini[15,0]  = -v_sq_s - v_sr  
    struct[0].g_ini[16,0]  = -epsilon_p + p_m - p_s  
    struct[0].g_ini[17,0]  = -epsilon_q - q_s + q_s_ref  
    struct[0].g_ini[18,0]  = i_sd*v_sd + i_sq*v_sq - p_s  
    struct[0].g_ini[19,0]  = i_sd*v_sq - i_sq*v_sd - q_s  
    struct[0].Fx_ini[0,1] = K_i 
    struct[0].Fx_ini[3,3] = -1.00000000000000e-5 
    struct[0].Fx_ini[3,4] = Omega_b 
    struct[0].Fx_ini[4,4] = -1.00000000000000e-5 
    struct[0].Fy_ini[0,16] = K_p 
    struct[0].Fy_ini[1,16] = 1 
    struct[0].Fy_ini[2,17] = 1 
    struct[0].Gx_ini[2,3] = -V_g*sin(phi_g) 
    struct[0].Gx_ini[3,3] = V_g*cos(phi_g) 
    struct[0].Gx_ini[8,0] = -i_sd_s*sin(phi_s) + i_sq_s*cos(phi_s) 
    struct[0].Gx_ini[9,0] = -i_sd_s*cos(phi_s) - i_sq_s*sin(phi_s) 
    struct[0].Gx_ini[11,2] = K_q/T_q 
    struct[0].Gx_ini[12,0] = -v_sd*sin(phi_s) - v_sq*cos(phi_s) 
    struct[0].Gx_ini[13,0] = v_sd*cos(phi_s) - v_sq*sin(phi_s) 
    ...
    struct[0].Gy_ini[8,6] = cos(phi_s) 
    struct[0].Gy_ini[8,7] = sin(phi_s) 
    struct[0].Gy_ini[8,8] = -1 
    struct[0].Gy_ini[9,6] = -sin(phi_s) 
    struct[0].Gy_ini[9,7] = cos(phi_s) 
    struct[0].Gy_ini[9,9] = -1 
    struct[0].Gy_ini[10,10] = -1 
    struct[0].Gy_ini[11,11] = -1 
    struct[0].Gy_ini[11,17] = K_q 
    struct[0].Gy_ini[12,10] = cos(phi_s) 
    struct[0].Gy_ini[12,11] = -sin(phi_s) 
    struct[0].Gy_ini[12,12] = -1 
    struct[0].Gy_ini[13,10] = sin(phi_s) 
    struct[0].Gy_ini[13,11] = cos(phi_s) 
    ...
    struct[0].Gy_ini[16,18] = -1 
    struct[0].Gy_ini[17,17] = -1 
    struct[0].Gy_ini[17,19] = -1 
    struct[0].Gy_ini[18,8] = v_sd 
    struct[0].Gy_ini[18,9] = v_sq 
    struct[0].Gy_ini[18,10] = i_sd 
    struct[0].Gy_ini[18,11] = i_sq 
    struct[0].Gy_ini[18,18] = -1 
    struct[0].Gy_ini[19,8] = v_sq 
    struct[0].Gy_ini[19,9] = -v_sd 
    struct[0].Gy_ini[19,10] = -i_sq 
    struct[0].Gy_ini[19,11] = i_sd 
...
def ini_struct(dt,values):
    ...
    values += [np.zeros((5,1))]
    dt +=     [('y_ini', np.float64, (20,1))]
    values += [np.zeros((20,1))]
    dt +=     [('f_ini', np.float64, (5,1))]
    values += [np.zeros((5,1))]
    dt +=     [('g_ini', np.float64, (20,1))]
    values += [np.zeros((20,1))]
    dt +=     [('Fx_ini', np.float64, (5,5))]
    values += [np.zeros((5,5))]
    dt +=     [('Fy_ini', np.float64, (5,20))]
    values += [np.zeros((5,20))]
    dt +=     [('Gx_ini', np.float64, (20,5))]
    values += [np.zeros((20,5))]
    dt +=     [('Gy_ini', np.float64, (20,20))]
    values += [np.zeros((20,20))]
...
if __name__ == "__main__":
    sys = {'t_end': 20.0, 'Dt': 0.01, 'solver': 'forward-euler', 'decimation': 10, 'name': 'vsg_pi', 'models': [{'params': {'K_p': 0.1, 'K_i': 0.01, 'K_q': 1.0, 'T_q': 1.0, 'Omega_b': 376.99111843077515, 'R_g': 0.01, 'X_g': 0.05, 'V_g': 1.0, 'H': 5.0}, 'f': ['dphi_s = K_p*epsilon_p + K_i*xi_p', 'dxi_p  = epsilon_p', 'dxi_q  = epsilon_q', 'dphi_g = Omega_b*(omega_g - 1) -1e-5*phi_g', 'domega_g = RoCoFpu - 1e-5*(omega_g - 1)'], 'g': ['p_h  @ p_h + 2*H*RoCoFpu', 'p_m0 @ p_m0 - p_m', 'v_gr @-v_gr + V_g*cos(phi_g)', 'v_gi @-v_gi + V_g*sin(phi_g)', 'i_sr @ - i_sr -(R_g*(v_sr - v_gr) + X_g*(v_si -v_gi))/(X_g**2 + R_g**2)', 'i_si @ - i_si -(R_g*(v_si - v_gi) - X_g*(v_sr -v_gr))/(X_g**2 + R_g**2)', 'i_sd_s@-i_sd_s + i_si', 'i_sq_s@-i_sq_s - i_sr', 'i_sd @-i_sd + cos(phi_s)*i_sd_s + cos(phi_s-pi/2)*i_sq_s', 'i_sq @-i_sq - sin(phi_s)*i_sd_s - sin(phi_s-pi/2)*i_sq_s', 'v_sd @ -v_sd + 0.0', 'v_sq @ -v_sq -1+ K_q*(epsilon_q + xi_q/T_q)', 'v_sd_s @ -v_sd_s + cos(-phi_s)*v_sd + cos(-phi_s-pi/2)*v_sq', 'v_sq_s @ -v_sq_s - sin(-phi_s)*v_sd - sin(-phi_s-pi/2)*v_sq', 'v_si@-v_sd_s + v_si', 'v_sr@-v_sq_s - v_sr', 'epsilon_p@-epsilon_p + p_m - p_s', 'epsilon_q@-epsilon_q + q_s_ref - q_s', 'p_s@-p_s+ i_sd*v_sd + i_sq*v_sq', 'q_s@-q_s+ i_sd*v_sq - i_sq*v_sd'], 'u': {'p_m': 0.8, 'q_s_ref': 0.1, 'RoCoFpu': 0.0}, 'y': ['p_h', 'p_m0', 'v_gr', 'v_gi', 'i_sr', 'i_si', 'i_sd_s', 'i_sq_s', 'i_sd', 'i_sq', 'v_sd', 'v_sq', 'v_sd_s', 'v_sq_s', 'v_sr', 'v_si', 'epsilon_p', 'epsilon_q', 'p_s', 'q_s'], 'y_ini': ['p_h', 'p_m0', 'v_gr', 'v_gi', 'i_sr', 'i_si', 'i_sd_s', 'i_sq_s', 'i_sd', 'i_sq', 'v_sd', 'v_sq', 'v_sd_s', 'v_sq_s', 'v_sr', 'v_si', 'epsilon_p', 'epsilon_q', 'p_s', 'q_s'], 'h': ['p_m']}], 'perturbations': [{'type': 'step', 'time': 100.0, 'var': 'p_m', 'final': 1.01}], 'itol': 1e-08, 'imax': 100, 'solvern': 1}
    syst =  vsg_pi_class()
