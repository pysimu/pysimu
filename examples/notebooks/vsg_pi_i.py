...
def run(t,struct, mode):
    ...
    v_sr = struct[it].y[6,0] 
    v_si = struct[it].y[7,0] 
    v_sd_s = struct[it].y[8,0] 
    v_sq_s = struct[it].y[9,0] 
    v_sd = struct[it].y[10,0] 
    v_sq = struct[it].y[11,0] 
    i_sd = struct[it].y[12,0] 
    i_sq = struct[it].y[13,0] 
    i_sd_s = struct[it].y[14,0] 
    i_sq_s = struct[it].y[15,0] 
    i_si = struct[it].y[16,0] 
    i_sr = struct[it].y[17,0] 
    ...
    if mode==3: # algebraic equations 
        ...
        struct[it].g[6,0] = R_g*i_sr - X_g*i_si + v_gr - v_sr  
        struct[it].g[7,0] = R_g*i_si + X_g*i_sr + v_gi - v_si  
        struct[it].g[8,0] = -v_sd_s + v_si  
        struct[it].g[9,0] = -v_sq_s - v_sr  
        struct[it].g[10,0] = -v_sd + v_sd_s*cos(phi_s) + v_sq_s*sin(phi_s)  
        struct[it].g[11,0] = -v_sd_s*sin(phi_s) - v_sq + v_sq_s*cos(phi_s)  
        struct[it].g[12,0] = R_s*i_sd + X_s*i_sq - v_sd  
        struct[it].g[13,0] = K_q*(epsilon_q + xi_q/T_q) + R_s*i_sq - X_s*i_sd - v_sq - 1  
        struct[it].g[14,0] = i_sd*cos(phi_s) - i_sd_s - i_sq*sin(phi_s)  
        struct[it].g[15,0] = i_sd*sin(phi_s) + i_sq*cos(phi_s) - i_sq_s  
        struct[it].g[16,0] = i_sd_s - i_si  
        struct[it].g[17,0] = -i_sq_s - i_sr  
    ...
    if mode==11: # Fy,Gx,Gy 
        ...
        struct[it].Gx[10,0] = -v_sd_s*sin(phi_s) + v_sq_s*cos(phi_s) 
        struct[it].Gx[11,0] = -v_sd_s*cos(phi_s) - v_sq_s*sin(phi_s) 
        struct[it].Gx[13,2] = K_q/T_q 
        struct[it].Gx[14,0] = -i_sd*sin(phi_s) - i_sq*cos(phi_s) 
        struct[it].Gx[15,0] = i_sd*cos(phi_s) - i_sq*sin(phi_s) 
        ...
        struct[it].Gy[6,4] = 1 
        struct[it].Gy[6,6] = -1 
        struct[it].Gy[6,16] = -X_g 
        struct[it].Gy[6,17] = R_g 
        struct[it].Gy[7,5] = 1 
        struct[it].Gy[7,7] = -1 
        struct[it].Gy[7,16] = R_g 
        struct[it].Gy[7,17] = X_g 
        struct[it].Gy[8,7] = 1 
        struct[it].Gy[8,8] = -1 
        struct[it].Gy[9,6] = -1 
        struct[it].Gy[9,9] = -1 
        ...
        struct[it].Gy[12,12] = R_s 
        struct[it].Gy[12,13] = X_s 
        struct[it].Gy[13,11] = -1 
        struct[it].Gy[13,12] = -X_s 
        struct[it].Gy[13,13] = R_s 
        ...
        struct[it].Gy[20,10] = i_sd 
        struct[it].Gy[20,11] = i_sq 
        struct[it].Gy[20,12] = v_sd 
        struct[it].Gy[20,13] = v_sq 
        struct[it].Gy[20,20] = -1 
        struct[it].Gy[21,10] = -i_sq 
        struct[it].Gy[21,11] = i_sd 
        struct[it].Gy[21,12] = v_sq 
        struct[it].Gy[21,13] = -v_sd 
...
def initialization(struct):
    ...
    struct[0].f_ini[0,0] = Omega_b*(omega_s - 1) 
    ...
    struct[0].g_ini[1,0]  = p_m - p_s_ref  
    ...
    struct[0].g_ini[6,0]  = R_g*i_sr - X_g*i_si + v_gr - v_sr  
    struct[0].g_ini[7,0]  = R_g*i_si + X_g*i_sr + v_gi - v_si  
    struct[0].g_ini[8,0]  = -v_sd_s + v_si  
    struct[0].g_ini[9,0]  = -v_sq_s - v_sr  
    struct[0].g_ini[10,0]  = -v_sd + v_sd_s*cos(phi_s) + v_sq_s*sin(phi_s)  
    struct[0].g_ini[11,0]  = -v_sd_s*sin(phi_s) - v_sq + v_sq_s*cos(phi_s)  
    struct[0].g_ini[12,0]  = R_s*i_sd + X_s*i_sq - v_sd  
    struct[0].g_ini[13,0]  = K_q*(epsilon_q + xi_q/T_q) + R_s*i_sq - X_s*i_sd - v_sq - 1  
    struct[0].g_ini[14,0]  = i_sd*cos(phi_s) - i_sd_s - i_sq*sin(phi_s)  
    struct[0].g_ini[15,0]  = i_sd*sin(phi_s) + i_sq*cos(phi_s) - i_sq_s  
    struct[0].g_ini[16,0]  = i_sd_s - i_si  
    struct[0].g_ini[17,0]  = -i_sq_s - i_sr  
    ...
    struct[0].Gx_ini[10,0] = -v_sd_s*sin(phi_s) + v_sq_s*cos(phi_s) 
    struct[0].Gx_ini[11,0] = -v_sd_s*cos(phi_s) - v_sq_s*sin(phi_s) 
    struct[0].Gx_ini[13,2] = K_q/T_q 
    struct[0].Gx_ini[14,0] = -i_sd*sin(phi_s) - i_sq*cos(phi_s) 
    struct[0].Gx_ini[15,0] = i_sd*cos(phi_s) - i_sq*sin(phi_s) 
    ...
    struct[0].Gy_ini[6,4] = 1 
    struct[0].Gy_ini[6,6] = R_g 
    struct[0].Gy_ini[6,7] = -X_g 
    struct[0].Gy_ini[6,16] = -1 
    struct[0].Gy_ini[7,5] = 1 
    struct[0].Gy_ini[7,6] = X_g 
    struct[0].Gy_ini[7,7] = R_g 
    struct[0].Gy_ini[7,17] = -1 
    struct[0].Gy_ini[8,14] = -1 
    struct[0].Gy_ini[8,17] = 1 
    struct[0].Gy_ini[9,15] = -1 
    struct[0].Gy_ini[9,16] = -1 
    struct[0].Gy_ini[10,12] = -1 
    struct[0].Gy_ini[10,14] = cos(phi_s) 
    struct[0].Gy_ini[10,15] = sin(phi_s) 
    struct[0].Gy_ini[11,13] = -1 
    struct[0].Gy_ini[11,14] = -sin(phi_s) 
    struct[0].Gy_ini[11,15] = cos(phi_s) 
    struct[0].Gy_ini[12,10] = R_s 
    struct[0].Gy_ini[12,11] = X_s 
    struct[0].Gy_ini[12,12] = -1 
    struct[0].Gy_ini[13,10] = -X_s 
    struct[0].Gy_ini[13,11] = R_s 
    struct[0].Gy_ini[13,13] = -1 
    struct[0].Gy_ini[13,19] = K_q 
    struct[0].Gy_ini[14,8] = -1 
    struct[0].Gy_ini[14,10] = cos(phi_s) 
    struct[0].Gy_ini[14,11] = -sin(phi_s) 
    struct[0].Gy_ini[15,9] = -1 
    struct[0].Gy_ini[15,10] = sin(phi_s) 
    struct[0].Gy_ini[15,11] = cos(phi_s) 
    struct[0].Gy_ini[16,7] = -1 
    struct[0].Gy_ini[16,8] = 1 
    struct[0].Gy_ini[17,6] = -1 
    struct[0].Gy_ini[17,9] = -1 
...
if __name__ == "__main__":
    sys = {'t_end': 20.0, 'Dt': 0.01, 'solver': 'forward-euler', 'decimation': 10, 'name': 'vsg_pi_i', 'models': [{'params': {'K_p': 0.01, 'K_i': 0.1, 'K_q': 0.1, 'T_q': 0.1, 'Omega_b': 314.1592653589793, 'R_g': 0.01, 'X_g': 0.05, 'V_g': 1.0, 'K_f': 0.0, 'K_s': 1.0, 'H': 5.0, 'R_s': 0.01, 'X_s': 0.01}, 'f': ['dphi_s = Omega_b*(omega_s - 1)', 'dxi_p  = epsilon_p', 'dxi_q  = epsilon_q', 'dphi_g = Omega_b*(omega_g - 1) -1e-5*phi_g', 'domega_g = RoCoFpu - 1e-5*(omega_g - 1)'], 'g': ['omega_s @ -omega_s + K_p*epsilon_p + K_i*xi_p + 1', 'p_s_ref @ -p_s_ref + p_m', 'p_h  @ p_h + 2*H*RoCoFpu', 'p_m0 @ p_m0 - p_m', 'v_gr @-v_gr + V_g*cos(phi_g)', 'v_gi @-v_gi + V_g*sin(phi_g)', 'v_sr @ -v_sr + v_gr + R_g*i_sr - X_g*i_si', 'v_si @ -v_si + v_gi + R_g*i_si + X_g*i_sr', 'v_sd_s@-v_sd_s + v_si', 'v_sq_s@-v_sq_s - v_sr', 'v_sd @-v_sd + cos(phi_s)*v_sd_s + cos(phi_s-pi/2)*v_sq_s', 'v_sq @-v_sq - sin(phi_s)*v_sd_s - sin(phi_s-pi/2)*v_sq_s', 'i_sd @ -v_sd    + 0.0                        + R_s*i_sd + X_s*i_sq', 'i_sq @ -v_sq -1 + K_q*(epsilon_q + xi_q/T_q) + R_s*i_sq - X_s*i_sd', 'i_sd_s @ -i_sd_s + cos(phi_s)*i_sd      - sin(phi_s)*i_sq', 'i_sq_s @ -i_sq_s + cos(phi_s-pi/2)*i_sd - sin(phi_s-pi/2)*i_sq', 'i_si@-i_si + i_sd_s', 'i_sr@-i_sr - i_sq_s', 'epsilon_p@-epsilon_p + p_s_ref - p_s', 'epsilon_q@-epsilon_q + q_s_ref - q_s', 'p_s@-p_s+ i_sd*v_sd + i_sq*v_sq', 'q_s@-q_s+ i_sd*v_sq - i_sq*v_sd'], 'u': {'p_m': 0.8, 'q_s_ref': 0.1, 'RoCoFpu': 0.0}, 'y': ['omega_s', 'p_s_ref', 'p_h', 'p_m0', 'v_gr', 'v_gi', 'i_sr', 'i_si', 'i_sd_s', 'i_sq_s', 'i_sd', 'i_sq', 'v_sd', 'v_sq', 'v_sd_s', 'v_sq_s', 'v_sr', 'v_si', 'epsilon_p', 'epsilon_q', 'p_s', 'q_s'], 'y_ini': ['omega_s', 'p_s_ref', 'p_h', 'p_m0', 'v_gr', 'v_gi', 'i_sr', 'i_si', 'i_sd_s', 'i_sq_s', 'i_sd', 'i_sq', 'v_sd', 'v_sq', 'v_sd_s', 'v_sq_s', 'v_sr', 'v_si', 'epsilon_p', 'epsilon_q', 'p_s', 'q_s'], 'h': ['p_m']}], 'perturbations': [{'type': 'step', 'time': 100.0, 'var': 'p_m', 'final': 1.01}], 'itol': 1e-08, 'imax': 100, 'solvern': 1}
    syst =  vsg_pi_i_class()
