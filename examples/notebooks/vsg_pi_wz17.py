...
if __name__ == "__main__":
    sys = {'t_end': 20.0, 'Dt': 0.01, 'solver': 'forward-euler', 'decimation': 10, 'name': 'vsg_pi_wz17', 'models': [{'params': {'K_p': 0.01, 'K_i': 0.1, 'K_q': 1.0, 'T_q': 1.0, 'Omega_b': 376.99111843077515, 'R_g': 0.01, 'X_g': 0.05, 'V_g': 1.0, 'K_f': 10.0, 'K_s': 1.0, 'H': 5.0}, 'f': ['dphi_s = Omega_b*(omega_s - 1)', 'dxi_p  = epsilon_p', 'dxi_q  = epsilon_q', 'dphi_g = Omega_b*(omega_g - 1) -1e-5*phi_g', 'domega_g = RoCoFpu - 1e-5*(omega_g - 1)'], 'g': ['omega_s @ -omega_s + K_p*epsilon_p + K_i*xi_p + 1', 'p_s_ref @ -p_s_ref + p_m', 'p_h  @ p_h + 2*H*RoCoFpu', 'p_m0 @ p_m0 - p_m', 'v_gr @-v_gr + V_g*cos(phi_g)', 'v_gi @-v_gi + V_g*sin(phi_g)', 'i_sr @ -i_sr -(R_g*v_gr - R_g*v_sr + X_g*v_gi - X_g*v_si)/(R_g**2 + X_g**2)', 'i_si @ -i_si -(R_g*v_gi - R_g*v_si - X_g*v_gr + X_g*v_sr)/(R_g**2 + X_g**2)', 'i_sd_s@-i_sd_s + i_si', 'i_sq_s@-i_sq_s - i_sr', 'i_sd @-i_sd + cos(phi_s)*i_sd_s + cos(phi_s-pi/2)*i_sq_s', 'i_sq @-i_sq - sin(phi_s)*i_sd_s - sin(phi_s-pi/2)*i_sq_s', 'v_sd @ -v_sd + 0.0', 'v_sq @ -v_sq -1+ K_q*(epsilon_q + xi_q/T_q)', 'v_sd_s @ -v_sd_s + cos(phi_s)*v_sd      - sin(phi_s)*v_sq', 'v_sq_s @ -v_sq_s + cos(phi_s-pi/2)*v_sd - sin(phi_s-pi/2)*v_sq', 'v_si@-v_si + v_sd_s', 'v_sr@-v_sr - v_sq_s', 'epsilon_p@-epsilon_p + p_s_ref + p_s', 'epsilon_q@-epsilon_q + q_s_ref - q_s', 'p_s@-p_s+ i_sd*v_sd + i_sq*v_sq', 'q_s@-q_s+ i_sd*v_sq - i_sq*v_sd'], 'u': {'p_m': 0.8, 'q_s_ref': 0.1, 'RoCoFpu': 0.0}, 'y': ['omega_s', 'p_s_ref', 'p_h', 'p_m0', 'v_gr', 'v_gi', 'i_sr', 'i_si', 'i_sd_s', 'i_sq_s', 'i_sd', 'i_sq', 'v_sd', 'v_sq', 'v_sd_s', 'v_sq_s', 'v_sr', 'v_si', 'epsilon_p', 'epsilon_q', 'p_s', 'q_s'], 'y_ini': ['omega_s', 'p_s_ref', 'p_h', 'p_m0', 'v_gr', 'v_gi', 'i_sr', 'i_si', 'i_sd_s', 'i_sq_s', 'i_sd', 'i_sq', 'v_sd', 'v_sq', 'v_sd_s', 'v_sq_s', 'v_sr', 'v_si', 'epsilon_p', 'epsilon_q', 'p_s', 'q_s'], 'h': ['p_m']}], 'perturbations': [{'type': 'step', 'time': 100.0, 'var': 'p_m', 'final': 1.01}], 'itol': 1e-08, 'imax': 100, 'solvern': 1}
    syst =  vsg_pi_wz17_class()
